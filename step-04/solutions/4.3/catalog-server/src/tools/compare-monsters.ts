...
export function registerCompareMonsters(server: McpServer) {
  server.registerTool(
    ...
    async ({ name_a, name_b }) => {
      ...
      logResult(
        "compare_monsters",
        `compared a="${a.name}" b="${b.name}" augments=${aAugS.length}/${bAugS.length} hindrances=${aHinS.length}/${bHinS.length}`,
        Date.now() - start,
      );
